    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {cart.map(item => (
          <div key={item.id} className="bg-white rounded-xl border p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-gray-500 text-sm">₹{Number(item.price).toLocaleString('en-IN')} each</p>
            </div>
      ...
      <div className="bg-white rounded-xl border p-6 mt-6">
        <div className="flex justify-between items-center text-lg">
          ...
          <span className="font-bold text-2xl text-primary-600">₹{Number(totalPrice).toLocaleString('en-IN')}</span>
        </div>
        {totalPrice < 500 && <p className="text-sm text-gray-500 mt-1">Add ₹{Number(500 - totalPrice).toLocaleString('en-IN')} more for free shipping</p>}
